...
test('intersects', () => {
    ...
    let getJustXY = (input: { X: number, Y: number }) => ({ X: input.X, Y: input.Y });

    for (let combination of combinations) {
        expect(intersects(combination[0], combination[1]).map(getJustXY)).toStrictEqual([
            { X: 1, Y: 1 }
        ]);
    }
});

test('distance from provided input', () => {
    const testCases = [
        {
