...
test('print grid', () => {
    let grid: XY[] = [
        { X: 5, Y: 5, hasPainted: true, value: 1 },
        { X: -5, Y: -5, hasPainted: true, value: 1 },
        { X: 5, Y: -5, hasPainted: true, value: 1 },
        { X: -5, Y: 5, hasPainted: true, value: 1 }
    ];
    expect(printGrid(grid)).toStrictEqual(`X         X
...
test('testMovements', () => {
    expect(walkOnGrid('', 0, [1, 1, 1, 1]).filter(x => x.hasPainted).length).toStrictEqual(4);
});
